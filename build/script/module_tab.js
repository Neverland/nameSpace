...
void function(){
	function Tab(O){
		...
		if(that.t.length===0 || !that.T || that.t.length!==that.s.length){return false};
		
		that.handler || (indicator.prototype.handler=function(){
			var i=that.t.length;
			for(;i>0;void function(j){
				that.addEvent(that.t[j],that.E,function(e){e=e || window.event;that.control(j);that.stopEvent(e);},false);
			}(--i)){};
		});
